// =================================================================
// 1. CONFIGURATION (Edit these default values)
// =================================================================

/**
 * Default UUID. This is the user ID for the NLESS protocol.
 * @type {string}
 * This value can be overridden by setting the `ID` environment variable in your Cloudflare worker settings.
 */
const userID = '23e0a1ab-f173-443c-ae26-1e3391a6ed4e';

/**
 * Default PROXYIP. This is the IP address of a proxy server to which the worker will forward traffic.
 * @type {string}
 * This value can be overridden by setting the `PROXYIP` environment variable.
 */
const proxyIP = '';

/**
 * Default UUID validation level.
 * `null`: (Recommended) Validates the full UUID.
 * `0`: Skips UUID validation.
 * `> 0`: Validates the first `idyes` characters of the UUID.
 * @type {number | null}
 * This value can be overridden by setting the `IDYES` environment variable.
 */
...
// Pre-compute the byte-to-hex lookup table for UUID stringification.
const byteToHex = [];
for (let i = 0; i < 256; ++i) {
...
// =================================================================
// 3. MAIN WORKER ENTRYPOINT
// =================================================================
export default {
	async fetch(request, env, ctx) {
		try {
			// Load configuration for this request, using environment variables if they exist,
			// otherwise falling back to the default values defined at the top of the file.
...
// =================================================================
// 5. PROTOCOL & UTILITY FUNCTIONS
// =================================================================

/**
 * Parses the NLESS protocol header from the first chunk of data.
 * @param {ArrayBuffer} nlessBuffer - The incoming data buffer.
 * @param {string} id - The expected UUID for validation.
 * @param {number|null} idyes - The validation mode (0: skip, >0: partial, null: full).
 * @returns {object} - Parsed header information or an error.
 */
...
/**
 * Handles UDP outbound traffic, specifically for DNS over HTTPS.
 * @param {WebSocket} webSocket - The client WebSocket.
 * @param {Uint8Array} nlessResponseHeader - The initial NLESS response header.
 * @returns {object} - An object with a 'write' function to send UDP data.
 */
 ...
 * @returns {object} - An object with 'earlyData' ArrayBuffer or an 'error'.
 */
...
/**
 * Safely closes a WebSocket connection, ignoring errors.
...
/**
 * Converts a 16-byte array into a hyphenated UUID string.
...
/**
 * Generates the NLESS configuration string for clients.
 * @param {string} id - The user UUID.
 * @param {string} hostName - The service hostname.
 * @returns {string} The NLESS configuration URL.
 */
