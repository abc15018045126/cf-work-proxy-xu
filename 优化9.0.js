...
let proxyIP = '';
let idyes = null; // Will be parsed from env.IDYES.nullor0or1
...
class ConnectionHandler {
	...
	async processFirstChunk(chunk) {
		...
		} = processNlessHeader(chunk, id, idyes);
...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			// Read and parse idyes from environment variables
			const idyesEnv = env.IDYES;
			if (idyesEnv !== undefined) {
				const parsedIdyes = parseInt(idyesEnv, 10);
				idyes = isNaN(parsedIdyes) ? null : parsedIdyes;
			} else {
				idyes = null;
			}
...
function processNlessHeader(
	...
	// Flexible user validation based on idyes
	if (idyes === 0) {
		// idyes is 0, skip validation.
	} else {
		const receivedId = stringify(new Uint8Array(nlessBuffer.slice(1, 17)));
		let isValidUser = false;
		if (idyes > 0) {
			// idyes is a positive number, validate the first 'idyes' characters.
			isValidUser = receivedId.substring(0, idyes) === id.substring(0, idyes);
		} else {
			// idyes is null or not a positive number, perform full validation.
