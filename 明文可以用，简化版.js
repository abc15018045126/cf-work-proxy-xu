	...
	readableWebSocketStream.pipeTo(new WritableStream({
		async write(chunk, controller) {
			...
			const {
				...
				addressRemote = '',
				rawDataIndex,
				nlessVersion = new Uint8Array([0, 0]),
				isUDP,
			...
			if (isDns) {
				const { write } = await handleUDPOutBound(webSocket, nlessResponseHeader, log);
				udpStreamWrite = write;
				udpStreamWrite(rawClientData);
				return;
			}
			handleTCPOutBound(remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, nlessResponseHeader, log);
		},
		close() {
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, nlessResponseHeader, log,) {
	...
	async function retry() {
		...
		remoteSocketToWS(tcpSocket, webSocket, nlessResponseHeader, null, log);
	}

	const tcpSocket = await connectAndWrite(addressRemote, portRemote);

	remoteSocketToWS(tcpSocket, webSocket, nlessResponseHeader, retry, log);
}
...
async function remoteSocketToWS(remoteSocket, webSocket, nlessResponseHeader, retry, log) {
		...
		.pipeTo(
			new WritableStream({
				...
				async write(chunk, controller) {
					...
					if (nlessHeader) {
						webSocket.send(await new Blob([nlessHeader, chunk]).arrayBuffer());
						nlessHeader = null;
					} else {
...
function stringify(arr, offset = 0) {
	const id_str = unsafeStringify(arr, offset);
	if (!isValidID(id_str)) {
...
async function handleUDPOutBound(webSocket, nlessResponseHeader, log) {
		...
		async write(chunk) {
			...
			if (webSocket.readyState === WS_READY_STATE_OPEN) {
				...
				if (isNlessHeaderSent) {
					...
					webSocket.send(await new Blob([nlessResponseHeader, udpSizeBuffer, dnsQueryResult]).arrayBuffer());
