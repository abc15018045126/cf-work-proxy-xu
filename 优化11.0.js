...
export default {
	async fetch(request, env, ctx) {
		try {
			// Configuration from environment variables, scoped to this request.
			const id = env.ID || '23e0a1ab-f173-443c-ae26-1e3391a6ed4e';
			let proxyIP = env.PROXYIP || '';
			const idyesEnv = env.IDYES;
			const idyes = idyesEnv !== undefined ? (parseInt(idyesEnv, 10) || null) : null;
			...
			const handler = new ConnectionHandler(server, request.headers.get('sec-websocket-protocol') || '', id, proxyIP, idyes);
			handler.handle();
