			...
			.pipeTo(
				new WritableStream({
					write: async (chunk) => {
						...
						const dataToSend = nlessHeader ? await new Blob([nlessHeader, chunk]).arrayBuffer() : chunk;
	...
	transformStream.readable.pipeTo(new WritableStream({
		async write(chunk) {
			...
			if (webSocket.readyState === WS_READY_STATE_OPEN) {
				const dataToSend = isNlessHeaderSent
					? await new Blob([udpSizeBuffer, dnsQueryResult]).arrayBuffer()
					: await new Blob([nlessResponseHeader, udpSizeBuffer, dnsQueryResult]).arrayBuffer();
