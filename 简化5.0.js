...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			if (path.includes('/proxyip=')) {
				proxyIP = path.split('/proxyip=')[1];
			}
...
async function nlessOverWSHandler(request) {
	...
	readableWebSocketStream.pipeTo(new WritableStream({
		async write(chunk, controller) {
			...
			if (isDns) {
				const { write } = await handleUDPOutBound(webSocket, nlessResponseHeader);
				udpStreamWrite = write;
				udpStreamWrite(rawClientData);
				return;
			}
			handleTCPOutBound(remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, nlessResponseHeader);
		},
		close() {
		},
		abort(reason) {
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, nlessResponseHeader) {
	...
	async function retry() {
		...
		remoteSocketToWS(tcpSocket, webSocket, nlessResponseHeader, null);
	}

	const tcpSocket = await connectAndWrite(proxyIP || addressRemote, portRemote);

	remoteSocketToWS(tcpSocket, webSocket, nlessResponseHeader, retry);
}
...
async function remoteSocketToWS(remoteSocket, webSocket, nlessResponseHeader, retry) {
